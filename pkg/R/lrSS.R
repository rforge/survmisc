##' @name lrSS
##' @export
##' @title Sample size required to show difference in survival by log-rank test given prior information about Kaplan-Meier estimate
##' @description No. of events required in a two-group trial (with one binary covariate) for a two-sided log-rank test to detect a given hazard ratio.
##' \cr
...
##' \cr \cr
##' This default calculation requires that the subjects be followed until \emph{all} have experienced the event of interest (typically death). If this is not likely to be the case, then a more informed estimate may be generated by dividing \eqn{n} by the overall probability of death occurring by the end of the study.
##' \cr
##' This may be generated with prior information about \eqn{\hat{S}}{S} and median survival times (for group \eqn{B} of groups \eqn{A} and \eqn{B}).
##' \cr \cr
##' Given accrual time \eqn{a} and follow-up time \eqn{f}, Simpsons rule can be used to give an estimate of the proportion of patients that will die in group \eqn{B}:
##' \deqn{ d_B = 1-\frac{1}{6}[\hat{S}_B(f) + 4\hat{S}_B(f+0.5a) + \hat{S}_B(f+a)] }{
##' dB = 1 - 1/6( S(f) +4S(f+0.5a) +S(f+a) )}
##' Given median survival time \eqn{t}, the proportion of patients expected to  die in group \eqn{B} is:
##' \deqn{ d_B = [1 - \frac{ \exp{ \frac{-0.69f}{t}} (1 - \exp{ \frac{-0.69f}{t}})}{\frac{0.69a}{t}}] }{
##' dB = 1 - ( exp(-0.69f/t) [1-exp(-0.69f/t)] / 0.69a/t )}
##' @param HR Hazard Ratio. Ratio of hazard with treatment to that without.
##' @param PRIOR if \code{TRUE} then additional estimates will be given
...
##' @param p Proportion of subjects allocated to one group. Arbitrary - can be either of the two groups.
##' @return
##' If \code{PRIOR = FALSE}, \bold{n}, the no. of subjects required (with no prior information).
##' \cr \cr
##' If \code{PRIOR = TRUE}, A list with the following values:
##' \describe{
##'  \item{n}{no. of subjects required (with no prior information)}
...
##'   \item{n}{no. of subjects required}
##'   }}
...
lrSS <- function(HR, PRIOR = FALSE,
                 Sp,tp,mtp,a,f,
                 alpha=0.1,beta=0.2,p=0.5){
### convert significance levels to Z values
    Za <- stats::qnorm(1-alpha/2)
    Zb <- stats::qnorm(1-beta)
    n1 <- (Za+Zb)^2/(log(HR)^2*p*(1-p))
    if (!PRIOR) return (n1)
### if prior info. present, check it is valid
    if (!length(Sp)==length(tp)) stop ("Must one survival estimate for each time point")
    err1 <- paste("Survival estimates must correspond to the following times:",f, 0.5*a+f,a+f,sep=" ")
